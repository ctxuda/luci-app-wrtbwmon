...
function clickToResetDatabase(settings) {
	if (confirm(_('This will delete the database file. Are you sure?'))) {
		getPath().then(function(res) {
			...
			execCmd('/bin/rm', [db]).then(function() {
...
function execCmd() {
	if (fs.exec_direct){
		return fs.exec_direct.apply(this, arguments);
	}
	else {
		return fs.exec.apply(this, arguments).then(function() {
			return res.stdout
		});
	}
}
...
function execRead() {
	if (fs.read_direct){
		return fs.read_direct.apply(this, arguments);
	}
	else {
		return fs.read.apply(this, arguments);
	}
}
...
function parseDefaultSettings(file) {
	return execRead(file).then(function(json) {
		var settings;
		try {
...
function resolveCustomizedHostName() {
	return L.resolveDefault(execRead(hostNameFile), []).then(function(hostNames) {
...
function updateData(table, updated, updating, settings) {
	updatePerSec(updating, settings.interval);
	if (!(L.Poll.tick % settings.interval)) {
		getPath()
		.then(function(res) {
			var params = settings.protocol == 'ipv4' ? '-4' : '-6';
			execCmd('/usr/sbin/wrtbwmon', [params, '-f', res]);
			return params == '-4' ? res : renameFile(res, '6');
		}).then(function(data) {
			Promise.all([
				execCmd('/bin/cat', [ data ]),
				resolveHostNameByMACAddr()
			]).then(function(res) {
				//console.time('start');
				cachedData = parseDatabase(res[0] || '', res[1], settings.showZero);
				displayTable(table, settings);
...
/*			L.Poll.add(function() {
				Promise.all([
					updateData.call(this, data[0], data[5], settings),
					updatePerSec($('updating'))
				])
			}, 1),*/
			L.Poll.add(updateData.bind(this, data[0], data[1], data[2], settings), 1);
			return data[0];
		})
